...
int x_detect_for_sprite_loop(int x_plus,int y,t_data *data,double angel,int ray_count)
{
    ...
    while(i < data->counter_of_sprites)
    {
        ...
        b[0] = floor( data->player_x + (data->player_mini_res / 2) - tpx);
        if (b[1] == data->pos_of_sprite_y[i] && b[0] == data->pos_of_sprite_x[i] && floor(b[1] / data->mini_map_res) <= y &&  
            floor(b[1] / data->mini_map_res) >= y  && floor(b[0] / data->mini_map_res) <= x_plus &&  floor(b[0] / data->mini_map_res) >= x_plus)
        {
          //  printf("hi\n");
            data->px_x = b[0]; 
            data->py_x =  b[1];
            ...
            //printf("d = %d  c = %d\n",data->table_pos_of_sprite_x[i][0],data->table_pos_of_sprite_x[i][1]);
        }
        i++;
    }
    return(0);
