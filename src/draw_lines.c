...
void	draw_linev4_for_static_imgs(t_data *data, int *begin, int *end,int high,char *addr,int line_len)
{
	...
	while(x < high)
	{
		...
		while (pixels)
		{
			if(y > 0)
			{
				pixel_color = get_pixel((t_get_pixel){addr,line_len,x ,y,high});
				...
				if(pixel[0] > 0  && pixel[0] <  WIN_H && pixel[1] > 0  && pixel[1] <  WIN_H && pixel_color > 0 )
					rander_image(&data->img, (t_rect){p0 ,p1 ,1, 1, pixel_color});
				y--;
			}
			pixel[0] += delta[0];
			pixel[1] += delta[1];
		...
		while (pixels)
		{
			if(y < data->coin_res - 1)
			{
				...
				if(y < data->coin_res && x < data->coin_res && y >= 0 && x >= 0)
					pixel_color = get_pixel((t_get_pixel){data->img_sprite.addr,data->img_sprite.line_len,x,y,data->coin_res});
				if(pixel[0] > 0  && pixel[0] <  WIN_H && pixel[1] > 0  && pixel[1] <  WIN_H && pixel_color > 0 )
				{
	...
	while (pixels && line_var.x < data->map_res)
	{
		if (line_var.y < data->map_res)
		{
			...
			if (line_var.y < data->map_res && line_var.x < data->map_res && line_var.y >= 0 && line_var.x >= 0)
				pixel_color = get_pixel((t_get_pixel){line_var.addr,line_var.line_len,line_var.x,line_var.y,data->map_res});
			if (pixel[0] > 0  && pixel[0] <  WIN_H && pixel[1] > 0  && pixel[1] <  WIN_H && pixel_color > 0)
				rander_image(&data->img, (t_rect){pixel[0], pixel[1],1, 1, pixel_color});
			temp_y++;
		}
