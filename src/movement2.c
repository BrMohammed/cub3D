...
/*   Updated: 2022/08/28 18:25:38 by brmohamm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	key_down_for_leters(t_data *data, int key)
{
	...
	if (key == 49)
			data->key_select[KEY_SPACE] = 1;
	if ((key == 2 || key == 0 || key == 13 || key == 1
			|| key == 36 || key == 46))
	{
		data->key_down[ON_KEYDOWN_DERECTION] = 1;
		if (key == 2)
			data->key_select[KEY_D] = 1;
		else if (key == 0)
			data->key_select[KEY_A] = 1;
		if (key == 13)
			data->key_select[KEY_W] = 1;
		else if (key == 1)
			data->key_select[KEY_S] = 1;
		if (key == 36)
...
int	key_down(int key, t_data *data)
{
	...
	{
		data->key_down[ON_KEYDOWN_CAMERA] = 1;
		if (key == 123)
			data->key_select[KEY_LEFT] = 1;
		else if (key == 124)
			data->key_select[KEY_WRIGHT] = 1;
		if (key == 125)
			data->key_select[KEY_DOWN] = 1;
		else if (key == 126)
			data->key_select[KEY_UP] = 1;
	}
	return (0);
}
...
void	key_up_for_leters(t_data *data, int key)
{
	...
	{
		data->key_down[ON_KEYDOWN_DERECTION] = 0;
		if (key == 2)
			data->key_select[KEY_D] = 0;
		else if (key == 0)
			data->key_select[KEY_A] = 0;
		if (key == 13)
			data->key_select[KEY_W] = 0;
		else if (key == 1)
			data->key_select[KEY_S] = 0;
		if (key == 36)
			data->key_select[KEY_ENTER] = 0;
	}
}
...
int	key_up(int key, t_data *data)
{
	...
	if (key == 123 || key == 124 || key == 126 || key == 125)
	{
		data->key_down[ON_KEYDOWN_CAMERA] = 0;
		if (key == 123)
			data->key_select[KEY_LEFT] = 0;
		else if (key == 124)
			data->key_select[KEY_WRIGHT] = 0;
		if (key == 125)
			data->key_select[KEY_DOWN] = 0;
		else if (key == 126)
			data->key_select[KEY_UP] = 0;
	}
	return (0);
}
